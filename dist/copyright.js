 ...
 */
export function enforceCopyright(article, corpus) {
    const violations = [];
    ...
    const bodyText = article.body
        ...
        .map((b) => { const tb = b; return tb.text ?? (tb.items ?? []).join(' '); })
        .join(' ');
    if (bodyText.trim()) {
    ...
    const fullText = [
        article.headline,
        article.dek,
        ...article.body.map((b) => { const tb = b; return tb.text ?? (tb.items ?? []).join(' '); }),
        article.legalNote,
...
export function isQuoteWithinLimit(text, maxWords = MAX_QUOTE_WORDS) {
 ...
 */
export function longestVerbatimRun(candidate, sources) {
    if (sources.length === 0)
        return 0;
    ...
    for (const source of sources) {
        ...
        for (let si = 0; si < srcWords.length; si++) {
            for (let ci = 0; ci < candWords.length; ci++) {
                if (srcWords[si] !== candWords[ci])
                    continue;
                // Words match at (si, ci) — extend the run
                let run = 1;
                while (si + run < srcWords.length &&
