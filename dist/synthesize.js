 ...
 */
import { SCHEMA_VERSION, CONTRACT_REVISION, assertCompatibleArtifact } from "./contracts.js";
import { buildDeterministicDraft } from "./provider.js";
import { planAssembly, toRenderBlocks, assembleArticle, buildChartBlocks } from "./assemble.js";
import { buildVoiceDirective } from "./style.js";
import { buildProvenanceFromFacts, buildProvenance, isFullyGrounded } from "./provenance.js";
import { enforceCopyright } from "./copyright.js";
import { validateRenderable } from "./render.js";
import { scrubUrl } from "./privacy.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/** Resolve the cluster members backing one Top-10 entry from the aggregation. */
export function resolveClusterMembers(entry, aggregation) {
...
function resolveMemberItems(memberIds, itemsByTopic) {
 ...
 */
function resolveFacts(entry, aggregation) {
 ...
 */
function extractClaimsFromBlocks(blocks) {
    const claims = [];
 ...
 */
function buildHeldArticle(entry, members, heldReason, upstreamRunId, now) {
    ...
    const heldDraft = buildDeterministicDraft({ topic: entry.topic, topicLabel: entry.topicLabel, headline: entry.headline, references: refs, voiceDirective: buildVoiceDirective() }, entry.confidence);
    const blocks = toRenderBlocks(plan, heldDraft.sections);
    const base = assembleArticle(plan, blocks, heldDraft, { provider: 'deterministic', model: 'rules/v1', status: 'fallback', reason: heldReason, generatedAt: now.toISOString() }, upstreamRunId, now);
    ...
    return { ...base, editorialStatus: 'held' };
}
/**
 ...
 */
function entryReferencesToItems(entry) {
...
function withItemClaims(article, itemClaims) {
...
export async function synthesizeOne(entry, ctx) {
    const warnings = [];
    ...
    const effectiveMembers = clusterMembers.length > 0 ? clusterMembers : entryReferencesToItems(entry);
    ...
    const deterministicDraft = buildDeterministicDraft({ topic: entry.topic, topicLabel: entry.topicLabel, headline: entry.headline, references: refs, voiceDirective: buildVoiceDirective() }, entry.confidence);
    ...
    const textBlocks = toRenderBlocks(plan, genResult.draft.sections);
    // Step 4: S4 — build chart blocks from real extracted numbers
    const chartBlocks = buildChartBlocks(clusterFacts, refs);
    // Insert charts after the first heading section for max readability
    const allBlocks = insertVisualBlocks(textBlocks, chartBlocks);
    ...
    if (hasFacts) {
        ...
        if (!provenanceResult.isGrounded) {
            ...
            warnings.push(`${ungroundedTexts.length} ungrounded claim(s) in entry ${entry.rank} — attempting re-ask`);
            if (provider.canGenerate()) {
                ...
                const reaskTextBlocks = toRenderBlocks(plan, reaskResult.draft.sections);
                const reaskAllBlocks = insertVisualBlocks(reaskTextBlocks, chartBlocks);
        ...
        const held = withItemClaims(buildHeldArticle(entry, effectiveMembers, 'no-facts-path', ctx.top10.runId, now), itemClaims);
        // #18: run gates on held articles (credential/copyright/render screen)
        return runFinalGates(held, effectiveMembers, entry.rank, warnings);
    }
}
/** Merge ClaimProvenance[] and facts into the article for the wire format. */
function buildPublishedArticle(base, claimProvenances, facts, itemClaims) {
    const merged = {
        ...base,
...
function insertVisualBlocks(textBlocks, visuals) {
    if (visuals.length === 0)
        return textBlocks;
...
async function runFinalGates(article, corpus, rank, warnings) {
...
export async function synthesizeCycle(ctx) {
    const { top10, aggregation, now } = ctx;
    const gateWarnings = [];
    const { warnings: w1 } = assertCompatibleArtifact(top10, 'top10');
    gateWarnings.push(...w1);
    const { warnings: w2 } = assertCompatibleArtifact(aggregation, 'aggregation');
    gateWarnings.push(...w2);
    const cycleWarnings = [...gateWarnings];
    if (top10.cycle.id !== aggregation.cycle.id) {
        cycleWarnings.push(`Cycle id mismatch: top10=${top10.cycle.id} aggregation=${aggregation.cycle.id} — weaving from Top-10 references only`);
    }
    const allEntries = [
        ...top10.data.global,
    ...
    const uniqueEntries = allEntries.filter((entry) => {
        if (seenClusterIds.has(entry.clusterId))
            return false;
    ...
    for (const entry of uniqueEntries) {
        ...
        if (article === null) {
            ...
            const status = article.editorialStatus;
            if (status === 'held') {
                held++;
    ...
    if (held > 0)
        warnings.push(`${held} article(s) held (AI unavailable or grounding failed)`);
